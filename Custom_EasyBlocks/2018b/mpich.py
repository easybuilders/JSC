...
class EB_MPICH(ConfigureMake):
    ...
    def extra_options(extra_vars=None):
        ...
        extra_vars.update({
            ...
            'disable_fortran_checks': [False, "Disable the check of fortran bits in the sanity check", CUSTOM],
        })
        return extra_vars
    ...
    def sanity_check_step(self, custom_paths=None, use_new_libnames=None, check_launchers=True):
        ...
        if self.cfg['disable_fortran_checks']:
            self.log.info("Removing fortran bits from sanity check...")
            for x in [ 'mpifort', 'fmpich', 'mpif90' ]:
                libnames = [ y for y in libnames if x not in y]
            for x in [ 'mpif77', 'mpif90' ]:
                bins = [ y for y in bins if x not in y]
            for x in [ 'mpif.h' ]:
                headers = [ y for y in headers if x not in y]
