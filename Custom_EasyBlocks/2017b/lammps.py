...
class EB_LAMMPS(MakeCp):
    ...
    def extra_options(extra_vars=None):
        ...
        extra_vars.update({
            ...
            'use_user_misc': [False, "Include the optional, user-contributed \"user-misc\" package", CUSTOM],
    ...
    def build_asphere(self, verbose=False, path=None):
        ...
        cmd = "make yes-asphere"
        (asphere_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return asphere_src_output
    ...
    def build_kspace(self, verbose=False, path=None):
        ...
        os.chdir(os.path.join(self.cfg['start_dir'], 'src'))
        cmd = "make yes-kspace"
        (kspace_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return kspace_src_output
    ...
    def build_manybody(self, verbose=False, path=None):
        ...
        """
        os.chdir(os.path.join(self.cfg['start_dir'], 'src'))
        cmd = "make yes-manybody"
        (manybody_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return manybody_src_output
    ...
    def build_mc(self, verbose=False, path=None):
        ...
        os.chdir(os.path.join(self.cfg['start_dir'], 'src'))
        cmd = "make yes-mc"
        (mc_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return mc_src_output
    ...
    def build_molecule(self, verbose=False, path=None):
        ...
        """
        os.chdir(os.path.join(self.cfg['start_dir'], 'src'))
        cmd = "make yes-molecule"
        (molecule_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return molecule_src_output
    ...
    def build_poems(self, verbose=False, path=None):
        ...
        os.chdir(os.path.join(self.cfg['start_dir'], 'src'))
        cmd = "make yes-poems"
        (poems_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return poems_src_output
    ...
    def build_reax(self, verbose=False, path=None):
        ...
        os.chdir(os.path.join(self.cfg['start_dir'], 'src'))
        cmd = "make yes-reax"
        (reax_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return reax_src_output
    ...
    def build_user_misc(self, verbose=False, path=None):
        ...
        """
        os.chdir(os.path.join(self.cfg['start_dir'], 'src/USER-MISC'))
        cmd = "sh ./Install.sh 1" # 1 is the mode for installing. 0/2 are for uninstalling/updating
        (user_misc_src_output, _) = run_cmd(cmd, path=path, log_all=True, simple=False, log_output=verbose)
        return user_misc_src_output
    ...
    def build_step(self, verbose=False, path=None):
        ...
        if self.toolchain.options.get('usempi', None):
            ...
            if self.toolchain.comp_family() in [toolchain.INTELCOMP]:
                ...
                makearg = 'intel_cpu'
                if self.toolchain.mpi_family() in [toolchain.INTELMPI]:
                    makearg += '_intelmpi'
                elif self.toolchain.mpi_family() in [toolchain.MPICH]:
                    makearg += '_mpich'
                elif self.toolchain.mpi_family() in [toolchain.OPENMPI]:
                    makearg += '_openmpi'
                else:
                    raise EasyBuildError("MPI toolchain not supported!")
