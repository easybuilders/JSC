...
class EB_Rmpi(RPackage):
    ...
    def run(self):
        ...
        mpi_ver = self.toolchain.get_software_version(self.toolchain.MPI_MODULE_NAME)[0]
        if (Rmpi_type == 'INTELMPI') and (LooseVersion(mpi_ver) >= LooseVersion('2019.0.0')):
            mpi_root = '%s/intel64' % self.toolchain.get_software_root(self.toolchain.MPI_MODULE_NAME)[0]
        else:
            mpi_root = self.toolchain.get_software_root(self.toolchain.MPI_MODULE_NAME)[0]
