...
class EB_Siesta(ConfigureMake):
    ...
    def sanity_check_step(self):
        ...
        if self.toolchain.options.get('usempi', None):
            environment = ''
            # To allow the test to run in the wrong context
            if self.toolchain.mpi_family() == toolchain.INTELMPI:
                environment = 'export I_MPI_FABRICS=shm && '
            ...
            mpi_test_cmd = environment
            mpi_test_cmd += "cd %s && " % self.builddir
            mpi_test_cmd += "echo 'SystemName test' | siesta 2>/dev/null | grep PARALLEL"
