...
class EB_Libint(CMakeMake):
    ...
    def extra_options():
        ...
        extra_vars = {
            ...
            'with_cxx': [True, "Enable CXX support", CUSTOM],
        }
    ...
    def configure_step(self):
        ...
        if self_version < '2.7.0':
            ...
            if self.cfg['with_fortran']:
                ...
                self.cfg.update('configopts', '-DENABLE_FORTRAN=OFF ')

            if self_version >= '2.11':
                if self.cfg['with_cxx']:
                    self.cfg.update('configopts', ' -DREQUIRE_CXX_API=ON ')
                else:
                    self.cfg.update('configopts', ' -DREQUIRE_CXX_API=OFF ')

            # also build shared libraries (not enabled by default)
            self.cfg.update('configopts', "-DLIBINT2_BUILD_SHARED_AND_STATIC_LIBS=ON -DBUILD_SHARED_LIBS=ON")
