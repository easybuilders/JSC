...
class EB_Clang(CMakeMake):
    ...
    def extra_options():
        extra_vars = {
            ...
            'stage2_configopts': [None, "Config options for stage 2 ", CUSTOM],
            'stage3_configopts': [None, "Config options for stage 3 ", CUSTOM],
        }
    ...
    def build_with_prev_stage(self, prev_obj, next_obj, stageconfigopts=None):
    ...
    def build_step(self):
        ...
        if self.cfg['bootstrap']:
            ...
            self.build_with_prev_stage(self.llvm_obj_dir_stage1, self.llvm_obj_dir_stage2, self.cfg['stage2_configopts'])
            self.run_clang_tests(self.llvm_obj_dir_stage2)

            self.log.info("Building stage 3")
            self.build_with_prev_stage(self.llvm_obj_dir_stage2, self.llvm_obj_dir_stage3, self.cfg['stage3_configopts'])
