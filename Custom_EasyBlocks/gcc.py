...
class EB_GCC(ConfigureMake):
    ...
    def create_dir(self, dirname):
        ...
        try:
            ...
            raise EasyBuildError(
                "Can't use dir %s to build in: %s", dirpath, err)
    ...
    def disable_lto_mpfr_old_gcc(self, objdir):
        ...
        if len(mpfr_dirs) == 1:
            ...
            if res:
                ...
                self.log.info(
                    "Found MPFR version %s (based name of MPFR source dir: %s)", mpfr_ver, mpfr_dir)
            else:
                self.log.warning(
                    "Failed to determine MPFR version from '%s', assuming v%s", mpfr_dir, mpfr_ver)
        else:
            self.log.warning(
                "Failed to isolate MPFR source dir to determine MPFR version, assuming v%s", mpfr_ver)
        ...
        if LooseVersion(mpfr_ver) >= LooseVersion('4.0'):
            ...
            if gcc_ver is None:
                ...
                regex_subs = [
                    (r'(--with-gmp-lib=\$\$r/\$\(HOST_SUBDIR\)/gmp/.libs) \\', r'\1 --disable-lto \\')]
    ...
    def prep_extra_src_dirs(self, stage, target_prefix=None):
        ...
        if LooseVersion(self.version) >= LooseVersion('4.5'):
            ...
            if stage not in known_stages:
                raise EasyBuildError(
                    "Incorrect argument for prep_extra_src_dirs, should be one of: %s", known_stages)
            ...
            for d in found_src_dirs:
                ...
                if not os.path.exists(dst):
                    try:
                        ...
                        raise EasyBuildError(
                            "Failed to copy src %s to dst %s: %s", src, dst, err)
                    self.log.debug("Copied %s to %s, so GCC can build %s" % (
                        src, dst, d['target_dir']))
                else:
                    self.log.debug(
                        "No need to copy %s to %s, it's already there." % (src, dst))
        else:
    ...
    def run_configure_cmd(self, cmd):
        ...
        (out, ec) = run_cmd("%s %s" %
                            (self.cfg['preconfigopts'], cmd), log_all=True, simple=False)

        if ec != 0:
            raise EasyBuildError(
                "Command '%s' exited with exit code != 0 (%s)", cmd, ec)
        ...
        if unknown_options:
            raise EasyBuildError(
                "Unrecognized options found during configure: %s", unknown_options)
    ...
    def configure_step(self):
        ...
        if self.cfg['withnvptx']:
            if self.iter_idx == 0:
                ...
                extra_source_dirs = glob.glob(os.path.join(
                    self.builddir, '%s-*' % extra_source))
                if len(extra_source_dirs) != 1:
                    raise EasyBuildError(
                        "Failed to isolate %s source dir" % extra_source)
                if self.iter_idx == 1:
                    # compile nvptx-tools
        ...
        if self.stagedbuild:
            ...
            self.stage1installdir = os.path.join(
                self.builddir, 'GCC_stage1_eb')
        ...
        if self.cfg['prefer_lib_subdir'] and not self.cfg['multilib']:
            ...
            self.log.info(
                "Patching MULTILIB_OSDIRNAMES in %s with '%s'", cfgfile, multilib_osdirnames)
    ...
    def build_step(self):
        ...
        if self.stagedbuild:
            ...
            cmd = "%s make %s %s" % (
                self.cfg['prebuildopts'], paracmd, self.cfg['buildopts'])
            ...
            stage2_info = self.prep_extra_src_dirs(
                "stage2", target_prefix=stage2prefix)
            ...
            for lib in ["gmp"] + self.with_dirs:
                ...
                if lib == "gmp" or self.cfg['with%s' % lib]:
                    ...
                    try:
                        ...
                        raise EasyBuildError(
                            "Failed to change to %s: %s", libdir, err)
                    if lib == "gmp":
                    ...
                    if lib == "gmp":
                        ...
                        env.setvar('CPPFLAGS', "%s -L%s -I%s " %
                                   (cppflags, libpath, incpath))
            ...
            if self.cfg['withppl']:
                ...
                for lib in ["lib64", "lib"]:
                    path = os.path.join(
                        self.stage1installdir, lib, "libstdc++.a")
    ...
    def post_install_step(self, *args, **kwargs):
        ...
        for key in COMP_CMD_SYMLINKS:
            ...
            if os.path.exists(target):
                ...
                raise EasyBuildError(
                    "Can't link '%s' to non-existing location %s", target, os.path.join(bindir, src))
        ...
        glob_pattern = os.path.join(
            self.installdir, 'lib*', 'gcc', '*-linux-gnu', self.version, 'include-fixed')
        ...
        if paths:
                ...
                for fn in retained_header_files:
                    ...
                    if os.path.exists(from_path):
                        if os.path.exists(to_path):
                            raise EasyBuildError(
                                "%s already exists, not overwriting it with %s!", to_path, from_path)
                        else:
                            copy_file(from_path, to_path)
                            self.log.info(
                                "%s copied to %s before renaming %s", from_path, to_path, include_fixed_path)
                    else:
            ...
            self.log.info(
                "No include-fixed subdirectory found at %s", glob_pattern)
    ...
    def sanity_check_step(self):
        ...
        glob_pattern = os.path.join(
            self.installdir, 'lib*', 'gcc', '*-linux-gnu', self.version)
        matches = glob.glob(glob_pattern)
        if matches:
            cands = nub([os.path.basename(os.path.dirname(x))
                         for x in matches])
            if len(cands) == 1:
                config_name_subdir = cands[0]
            else:
                raise EasyBuildError(
                    "Found multiple candidates for configuration name: %s", ', '.join(cands))
        else:
            raise EasyBuildError(
                "Failed to determine configuration name: no matches for '%s'", glob_pattern)
        ...
        if os_type == 'Linux':
            ...
            if LooseVersion(self.version) < LooseVersion("4.9.0"):
                lib_files.extend(["libmudflap.%s" %
                                  sharedlib_ext, "libmudflap.a"])
            else:
        ...
        if 'fortran' in self.cfg['languages']:
            ...
            lib_files.extend(['libgfortran.%s' %
                              sharedlib_ext, 'libgfortran.a'])
        ...
        if self.cfg['multilib']:
            ...
            lib_files64 = [os.path.join(libdir, x)
                           for libdir in libdirs64 for x in lib_files]
            lib_files32 = [tuple([os.path.join(libdir, x)
                                  for libdir in libdirs32]) for x in lib_files]
            lib_files = lib_files64 + lib_files32
        else:
            # lib64 on SuSE and Darwin, lib otherwise
            lib_files = [tuple([os.path.join(libdir, x)
                                for libdir in libdirs]) for x in lib_files]
        # lib on SuSE, libexec otherwise
        libdirs = ['libexec', 'lib']
        common_infix = os.path.join('gcc', config_name_subdir, self.version)
        libexec_files = [tuple([os.path.join(d, common_infix, x)
                                for d in libdirs]) for x in libexec_files]
