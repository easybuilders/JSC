...
FFTW_CPU_FEATURE_FLAGS_SINGLE_DOUBLE = [
    'avx', 'avx2', 'avx512', 'fma4', 'sse2', 'vsx']
# Altivec (POWER), SSE (x86), NEON (ARM), FMA (x86_64)
# asimd is CPU feature for extended NEON on AARCH64
FFTW_CPU_FEATURE_FLAGS = FFTW_CPU_FEATURE_FLAGS_SINGLE_DOUBLE + \
    ['altivec', 'asimd', 'neon', 'sse', 'sve']
FFTW_PRECISION_FLAGS = ['single', 'double', 'long-double', 'quad-precision']


class EB_FFTWAMD(ConfigureMake):
    ...
    def extra_options():
        ...
        extra_vars = {
            ...
            'use_amd_opt': [True, "Configure with optimizations from the AMD fork", CUSTOM],
        ...
        for prec in FFTW_PRECISION_FLAGS:
            help_msg = "Enable building of %s precision library" % prec.replace(
                '-precision', '')
            extra_vars[EB_FFTWAMD._prec_param(prec)] = [True, help_msg, CUSTOM]

        return ConfigureMake.extra_options(extra_vars)

    def __init__(self, *args, **kwargs):
        """Initialisation of custom class variables for FFTW."""
        super(EB_FFTWAMD, self).__init__(*args, **kwargs)
        ...
        for flag in FFTW_CPU_FEATURE_FLAGS:
            ...
            if flag == 'fma4' and self.cfg['use_fma4'] is None and self.cfg['use_fma'] is not None:
                self.log.deprecated(
                    "Use 'use_fma4' instead of 'use_fma' easyconfig parameter", '4.0')
                self.fma4 = self.cfg['use_fma']

        self.amd_opt = self.cfg['use_amd_opt']
        ...
        if self.cfg['auto_detect_cpu_features']:
            ...
            self.log.info(
                "CPU features considered for auto-detection: %s", cpu_features)
            ...
            for flag in cpu_features:
                ...
                if getattr(self, flag) is None and flag in avail_cpu_features:
                    self.log.info(
                        "Enabling use of %s (should be supported based on CPU features)", flag.upper())
        ...
        if self.cfg['with_quad_prec'] and cpu_arch in [AARCH32, AARCH64, POWER]:
            ...
            self.log.debug(
                "Quad-precision automatically disabled; not supported on %s.", cpu_arch)
    ...
    def run_all_steps(self, *args, **kwargs):
        ...
        for prec in FFTW_PRECISION_FLAGS:
            if self.cfg[EB_FFTWAMD._prec_param(prec)]:
                ...
                if self.sve:
                    ...
                    if prec == 'single' or prec == 'double':
                        prec_configopts.append(
                            '--enable-fma --enable-sve --enable-armv8-cntvct-el0')
                ...
                if self.amd_opt:
                    if prec == 'single' or prec == 'double':
                        prec_configopts.append('--enable-amd-fast-planner')
                        if not self.cfg['with_mpi']:
                            prec_configopts.append('--enable-amd-app-opt')

                    prec_configopts.append('--enable-amd-opt')
                    if self.cfg['with_mpi']:
                        prec_configopts.append('--enable-amd-mpifft')
                ...
                if cpu_arch == POWER and comp_fam == TC_CONSTANT_GCC:
                    ...
                    if prec == 'double' and fftw_ver <= LooseVersion('3.3.6'):
                        self.log.info(
                            "Disabling vsx for double precision on POWER with GCC for FFTW/%s" % self.version)
                ...
                self.cfg.update('configopts', [' '.join(
                    prec_configopts) + ' ' + common_config_opts])

        self.log.debug(
            "List of configure options to iterate over: %s", self.cfg['configopts'])

        return super(EB_FFTWAMD, self).run_all_steps(*args, **kwargs)
    ...
    def test_step(self):
        ...
        if self.toolchain.mpi_family() is not None and not build_option('mpi_tests'):
            self.log.info(
                "Skipping testing of FFTW since MPI testing is disabled")
            return
        ...
        super(EB_FFTWAMD, self).test_step()
    ...
    def sanity_check_step(self):
            ...
            if self.cfg['with_%s_prec' % prec]:
                ...
                for variant in ['', 'mpi', 'openmp', 'threads']:
                    ...
                    if variant == '' or self.cfg['with_%s' % variant] and not (prec == 'quad' and variant == 'mpi'):
                        extra_files.append(
                            'lib/libfftw3%s%s.a' % (letter, suff))
                        if self.cfg['with_shared']:
                            extra_files.append(
                                'lib/libfftw3%s%s.%s' % (letter, suff, shlib_ext))
        ...
        super(EB_FFTWAMD, self).sanity_check_step(custom_paths=custom_paths)
