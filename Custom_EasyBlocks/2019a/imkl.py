    ...
    def post_install_step(self):
        ...
        if self.cfg['interfaces']:
            ...
            if get_software_root('PGI'):
                ...
                # remove generic target processor. It fails in PGI >=19 with:
                # pgcc-Error-Unified binary is not currently supported with LLVM code generation: -tp x64
                # It is anyway unnecessary if the compiler node is the same as the node used for running
                regex_subs = [
                    ('-tp x64', ''),
                ]
                for lib in fftw2libs + fftw3libs:
                    apply_regex_substitutions(os.path.join(interfacedir, lib, 'makefile'), regex_subs)
