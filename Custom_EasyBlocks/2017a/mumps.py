...
class EB_MUMPS(ConfigureMake):
    ...
    def configure_step(self):
        ...
        # Choose correct math libraries
        if self.toolchain.options.get('openmp', None):
            libblas = "-L$BLAS_LIB_DIR $LIBBLAS_MT"
            scalap = "-L$SCALAPACK_LIB_DIR $LIBSCALAPACK_MT"
            libpar = "-L$SCALAPACK_LIB_DIR $LIBSCALAPACK_MT"
            # Enable BLR_MT if necessary
            if (parmetis or metis or scotch):
                optf = ' '.join([optf, '-DBLR_MT'])
        else:
