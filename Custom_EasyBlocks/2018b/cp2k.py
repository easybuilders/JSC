...
class EB_CP2K(EasyBlock):
    ...
    def extra_options():
        extra_vars = {
            ...
            'enable_library': [True, "Enable the creation of CP2K as a library", CUSTOM],
        }
    ...
    def build_step(self):
        ...
        # Optionally build CP2K as a library
        if self.cfg["enable_library"]:
    ...
    def install_step(self):
        ...
        if self.cfg['enable_library']:
            ...
            lib = os.path.join(self.cfg['start_dir'], 'lib', self.typearch, self.cfg['type'], 'libcp2k.a')
            try:
                if not os.path.exists(targetdir):
                    os.makedirs(targetdir)
                copy_file(lib, targetdir)
            except (OSError, IOError), err:
                raise EasyBuildError("Failed to copy library: %s", err)
