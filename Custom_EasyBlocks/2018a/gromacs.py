        ...
        if LooseVersion(self.version) < LooseVersion('4.6'):
            ...
            if get_software_root('imkl'):
                ...
                if comp_fam == toolchain.GCC:
                    mkl_libs = ['libmkl_gnu_thread.so']
                elif comp_fam == toolchain.INTELCOMP:
                    mkl_libs = ['libmkl_intel_thread.so']
                else: # Assume PGI
                    mkl_libs = ['libmkl_pgi_thread.so']
                mkl_libs.extend(['libmkl_intel_ilp64.so', 'libmkl_core.so'])
                mkl_libs = ([os.path.join(os.getenv('LAPACK_LIB_DIR'), lib) for lib in mkl_libs])
