...
class EB_Hypre(ConfigureMake):
    ...
    def extra_options():
        extra_vars = {
            'withcuda': [False, "Build with CUDA support", CUSTOM],
        }
    ...
    def configure_step(self):
        ...
        for dep in ["BLAS", "LAPACK"]:
            ...
            blas_libs = blas_libs.replace('-l', '')
            self.cfg.update('configopts', '--with-%s-libs="%s"' %
                            (dep.lower(), blas_libs))
            self.cfg.update('configopts', '--with-%s-lib-dirs="%s"' % (dep.lower(),
                                                                       os.getenv('%s_LIB_DIR' % dep)))

        # Use MPI implementation from EB
        self.cfg.update('configopts', '--with-MPI-include=%s' %
                        os.getenv('MPI_INC_DIR'))

        if self.cfg['withcuda']:
            if get_software_root('CUDA'):
                self.cfg.update('configopts', '--with-cuda')

                cuda_cc = build_option(
                    'cuda_compute_capabilities') or self.cfg['cuda_compute_capabilities']
                if not cuda_cc:
                ...
                cuda_cc_string = ' '.join(
                    [x.replace('.', '') for x in cuda_cc])
                self.cfg.update(
                    'configopts', '--with-gpu-arch="%s"' % cuda_cc_string)
